...
RSpec.describe "EventSubscriptionSpike", type: :spike do
  ...
  class CardExchangedEvent
    ...
    # イベント発行時のメッセージ
    def publish
      puts "メッセージ送信されました: game_id=#{@game_id}, player_id=#{@player_id}"
    end
  end
  ...
  class EventBroker
    ...
    def relay(payload)
      puts "メッセージを仲介しました: game_id=#{payload[:game_id]}, player_id=#{payload[:player_id]}"
    end
  end
  ...
  class EventSubscriber
    ...
    def handle(event)
      ...
      puts "メッセージを受信しました: game_id=#{event[:game_id]}, player_id=#{event[:player_id]}"
    end
  end
