...
module HandSet
  class SameRankStrengthComparer
    class RankNumbersCompareBuilder
      class << self
        def from_pair_values(hand_set1, hand_set2)
          pair_values1 = extract_pair_values(hand_set1)
          pair_values2 = extract_pair_values(hand_set2)
          Compare.new(pair_values1, pair_values2)
        end

        def from_three_of_a_kind_value(hand_set1, hand_set2)
          three_value1 = extract_three_of_a_kind_value(hand_set1)
          three_value2 = extract_three_of_a_kind_value(hand_set2)
          Compare.new(three_value1 ? [three_value1] : [], three_value2 ? [three_value2] : [])
        end

        def from_four_of_a_kind_value(hand_set1, hand_set2)
          four_value1 = extract_four_of_a_kind_value(hand_set1)
          four_value2 = extract_four_of_a_kind_value(hand_set2)
          Compare.new(four_value1 ? [four_value1] : [], four_value2 ? [four_value2] : [])
        end

        def from_straight_high_card(hand_set1, hand_set2)
          high_card1 = extract_straight_high_card(hand_set1)
          high_card2 = extract_straight_high_card(hand_set2)
          Compare.new([high_card1], [high_card2])
        end
        ...
        def extract_straight_high_card(hand_set)
